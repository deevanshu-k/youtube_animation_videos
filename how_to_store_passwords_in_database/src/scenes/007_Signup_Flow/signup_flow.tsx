...
export default makeScene2D(function* (view) {
    ...
    // Sign Up form
    const signUpRectRef = createRef<Rect>();
    const signUpBtnRectRef = createRef<Rect>();
    view.add(
        <Rect
            ref={signUpRectRef}
            layout
    ...
    yield* sequence(
        ...
        all(
            signUpBtnRectRef().fill(DEFAULT, 0),
            ...signUpBtnRectRef()
                .childrenAs<Txt>()
                .map((c) => c.fill("white", 0))
        )
    );
    yield* all(signUpRectRef().x(-600, 1), signUpRectRef().y(-200, 1));
